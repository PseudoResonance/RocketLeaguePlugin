...
/**	private static RLStatsAPI stats = null;
	...
	public void onCommand(MessageReceivedEvent e, String command, String[] args) {
		if (args.length == 0) {
			...
			if (args[0].equalsIgnoreCase("token")) {
				...
				case "platforms":
					CompletableFuture<APIRequest<Long, List<PlatformInfo>>> platforms = stats.getPlatforms(e.getMessageIdLong());
					...
					return;
				case "seasons":
					CompletableFuture<APIRequest<Long, List<Season>>> seasons = stats.getSeasons(e.getMessageIdLong());
				...
				case "leaderboard":
					if (args.length > 1) {
						...
						if (stat != null) {
							CompletableFuture<APIRequest<Long, List<Player>>> leaderboard = stats.getStatLeaderboard(stat, e.getMessageIdLong());
				...
				case "ranks":
					if (args.length > 1) {
						try {
							...
							CompletableFuture<APIRequest<Long, APIRequest<Integer, List<Tier>>>> tiers = stats.getTiers(season, e.getMessageIdLong());
						...
						CompletableFuture<APIRequest<Long, List<Tier>>> tiers = stats.getTiers(e.getMessageIdLong());
				...
				case "playlists":
					if (args.length > 1) {
						...
						if (pl != null) {
							CompletableFuture<APIRequest<Long, List<Playlist>>> playlists = stats.getPlaylistInfo(e.getMessageIdLong());
					...
					if (args.length >= 2) {
						...
						if (pl != null) {
							if (pl == Platform.PS4 || pl == Platform.XBOX) {
								...
								CompletableFuture<APIRequest<Long, Player>> search = stats.getPlayer(args[2], pl, e.getMessageIdLong());
								...
								if (args[2].length() >= 3) {
									...
									if (id != 0) {
										CompletableFuture<APIRequest<Long, Player>> search = stats.getPlayer(String.valueOf(id), pl, e.getMessageIdLong());
										search.thenAccept(this::notifyPlayer);
										returnQueue.put(e.getMessageIdLong(), e.getChannel());
										search.exceptionally(ex -> {if (ex.getCause() instanceof IllegalStateException) {if (ex.getCause().getMessage().equals("Unauthorized. API key is wrong. (E:401)")) {e.getChannel().sendMessage("Please add a valid Rocket League API token!").queue(); return null;} else {ex.printStackTrace(); e.getChannel().sendMessage("An error has occurred! Please try again!").queue();}} else if (ex.getCause() instanceof NullPointerException) e.getChannel().sendMessage("Player not found!").queue(); returnQueue.remove(e.getMessageIdLong()); return null;});
										return;
							...
							CompletableFuture<APIRequest<Long, SearchResultPage>> search = stats.searchPlayers(args[1], e.getMessageIdLong());
	...
	void notifyPlatforms(APIRequest<Long, List<PlatformInfo>> data) {
	...
	void notifySeasons(APIRequest<Long, List<Season>> data) {
		MessageChannel channel = returnQueue.remove(data.getID());
	...
	void notifyTiers(APIRequest<Long, List<Tier>> data) {
		MessageChannel channel = returnQueue.remove(data.getID());
		EmbedBuilder embed = new EmbedBuilder();
		embed.setColor(new Color(6, 128, 211));
		embed.setFooter("https://rocketleaguestats.com/", null);
	...
	void notifyTiersSeason(APIRequest<Long, APIRequest<Integer, List<Tier>>> data) {
		MessageChannel channel = returnQueue.remove(data.getID());
		EmbedBuilder embed = new EmbedBuilder();
		embed.setColor(new Color(6, 128, 211));
		embed.setFooter("https://rocketleaguestats.com/", null);
		String ret = "";
		boolean first = true;
		APIRequest<Integer, List<Tier>> value = data.getValue();
		int season = value.getID();
		HashMap<Integer, String> tierMap = new HashMap<Integer, String>();
		for (Tier t : value.getValue()) {
			if (season == RLCommand2.season)
				tierMap.put(t.getId(), t.getName());
			if (first)
				first = false;
			else
	...
	void notifyPlaylists(APIRequest<Long, List<Playlist>> data) {
		MessageChannel channel = returnQueue.remove(data.getID());
		List<Playlist> playlists = data.getValue();
		sendPlaylists(channel, playlists, null);
	}

	void notifyPlaylistsPlatform(APIRequest<Long, List<Playlist>> data) {
		MessageChannel channel = returnQueue.remove(data.getID());
		List<Playlist> playlists = data.getValue();
	...
	void notifyLeaderboard(APIRequest<Long, List<Player>> data) {
		MessageChannel channel = returnQueue.remove(data.getID());
		EmbedBuilder embed = new EmbedBuilder();
		embed.setColor(new Color(6, 128, 211));
		embed.setFooter("https://rocketleaguestats.com/", null);
	...
	void notifyPlayer(APIRequest<Long, Player> data) {
		MessageChannel channel = returnQueue.remove(data.getID());
		int season = -1;
	...
	void notifySearch(APIRequest<Long, SearchResultPage> data) {
		MessageChannel channel = returnQueue.remove(data.getID());
	...
	private static void sendPlayer(MessageChannel channel, Player p, int seasonID) {
		...
		if (System.currentTimeMillis() - tiersUpdate >= 86400000) {
			CompletableFuture<APIRequest<Long, List<Tier>>> tiers = RLCommand2.stats.getTiers(0L);
			try {
				APIRequest<Long, List<Tier>> ret = tiers.get();
				for (Tier t : ret.getValue()) {
					tierMap.put(t.getId(), t.getName());
				}
				RLCommand2.tiers = tierMap;
				tiersUpdate = System.currentTimeMillis();
	...
	private static void sendPlayer(MessageChannel channel, Player p) {
		...
		if (System.currentTimeMillis() - seasonUpdate >= 86400000) {
			CompletableFuture<APIRequest<Long, List<Season>>> seasons = RLCommand2.stats.getSeasons(0L);
			try {
				APIRequest<Long, List<Season>> ret = seasons.get();
				RLCommand2.season = ret.getValue().size();
				seasonUpdate = System.currentTimeMillis();
			} catch (InterruptedException | ExecutionException e) {
				e.printStackTrace();
			}
		...
		SeasonInfo info = p.getSeasonInfo(RLCommand2.season);
		if (info != null) {
			season += getRankedInfo(info.getAllPlaylistInfo());
		} else {
			season += "Did Not Participate";
		}
		embed.addField("Season " + RLCommand2.season + ":", season, true);
		channel.sendMessage(embed.build()).queue();
	}
	...
	public String getDesc(long id) {
		return Language.getMessage(id, "gameutils.rlCommandDescription");
	}
